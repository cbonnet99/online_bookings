...
class TaskUtilsTest < ActiveSupport::TestCase
  ...
  def test_send_reminders
    ...
    pro = Factory(:practitioner)
    booking_remind_me = Factory(:booking, :practitioner => pro, :starts_str => Booking.starts_str_builder(date_within_24_hours))
    ...
    booking_dont_remind_me = Factory(:booking, :practitioner => pro, :starts_str => Booking.starts_str_builder(date_after_24_hours))
  ...
  def test_send_pro_reminders
    ...
    remind_me = Factory(:booking, :starts_str => Booking.starts_str_builder(1.day.from_now), :practitioner_id => pro.id)
    dont_remind_me_too_late = Factory(:booking, :starts_str => Booking.starts_str_builder(2.days.from_now), :practitioner_id => pro.id)
    dont_remind_me_already_sent = Factory(:booking, :starts_str => Booking.starts_str_builder(1.day.from_now), :pro_reminder_sent_at => Time.now, :practitioner_id => pro.id)
